	...
	public OrthologSet[] getOrthologSet(int minl){
		...
		for(int i = 0; i< homologs.length; i++){
			if(homologs[i]!=null ){
				...
				for(int g = 0; g< genomeNumber; g++){
					for(int k = 0; k<homologs[i].genepairs.length; k++){
						if(homologs[i].genepairs[k]!=null){
							if(homologs[i].genepairs[k].gene1.genomeIndex == g){
								if(gns[g].equals("Missing")){
									gns[g] = homologs[i].genepairs[k].gene1.name;
								}else{
									if( inTheString(gns[g],homologs[i].genepairs[k].gene1.name)==false){
										gns[g] =gns[g]+"|"+ homologs[i].genepairs[k].gene1.name;
									}
								}
							}
							if(homologs[i].genepairs[k].gene2.genomeIndex == g){
								if(gns[g].equals("Missing")){
								gns[g] = homologs[i].genepairs[k].gene2.name;
								}else{
									if( inTheString(gns[g],homologs[i].genepairs[k].gene2.name)==false){
									gns[g] = gns[g]+"|"+homologs[i].genepairs[k].gene2.name;
									}
								}
							}
						}
